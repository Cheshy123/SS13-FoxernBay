...
/obj/machinery/media/jukebox/New()
	...
	var/file_list = flist("sound/jukebox/")
	var/regex/r = regex("(.*)\\.ogg")

	for(var/file in file_list)
		if(r.Find(file) != 0)
			tracks += new/datum/track(r.group[1], file("sound/jukebox/" + file))
...
/obj/machinery/media/jukebox/Topic(href, href_list)
	if(href_list["change_track"])
		...
		if(emagged)
			...
			StartPlaying()
	else if (href_list["change_volume"])
		var/diff = text2num(href_list["change_volume"])
		AdjustVolume(volume + diff)
	return 1
...
/obj/machinery/media/jukebox/proc/StartPlaying()
	...
	sound_token = sound_player.PlayLoopingSound(src, sound_id, current_track.sound, volume = volume, range = 12, falloff = 3, prefer_mute = TRUE)
