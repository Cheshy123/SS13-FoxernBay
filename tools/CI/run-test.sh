...
#    pip install --user beautifulsoup4 -q
    ...
    run_test "check changelog example unchanged" "md5sum -c - <<< '0a3d0f2c7501331ac25fc62354c282b0 *html/changelogs/example.yml'"
...
function run_byond_tests {
    ...
    if [[ "$CI" == "true" ]]; then
        ...
        tools/CI/install-byond.sh || exit 1
...
#	run_test_ci "check globals build" "python tools/GenerateGlobalVarAccess/gen_globals.py baystation12.dme code/_helpers/global_access.dm"
    run_test "build map unit tests" "tools/CI/dm.sh -DUNIT_TEST -M$MAP_PATH baystation12.dme"
